    ...
    STUFF(
        (
            ...
            WHERE
                ...
                --AND (r.[startDate] <= getdate()) -- DE doesn't appear to have a start date on current classes, but activeTrial limits to the active calendar at least.
                AND (r.[endDate] is null OR r.[endDate] >= getdate()) --non-ended roster enrollments only
...
WHERE
    ...
    AND (s.[endDate] IS NULL OR s.[endDate] >= getdate() OR s.[endStatus] NOT IN ('W1','W2','W2T','W3','W4','W6')) --end date is null or future or end status isn't real
