...
join dbo.Course crs ON crs.calendarID = cal.calendarID and crs.stateCode in ('99329P0','99322P0','99359P0') --only PK Courses
...
where 1=1
and ssh.staffType = 'P'
and (ssh.startDate IS NULL OR ssh.startDate <= '06/30/2026')
and (ssh.endDate IS NULL OR ssh.endDate >= '07/01/2025')
...
from dbo.District d
join dbo.School s ON s.districtID = d.districtID
join dbo.SchoolYear sy ON sy.endYear='2026'--sy.active = 1
join dbo.Calendar cal ON cal.endYear = sy.endYear and cal.schoolID = s.schoolID
...
and ea.startDate <= '06/30/2026'
and (ea.endDate IS NULL)
and (
	exists(select 1
			from dbo.Course crs
			where crs.calendarID = cal.calendarID
			and crs.stateCode in ('99329P0','99322P0','99359P0') --only PK Courses
			and crs.active = 1
			)
	or exists(
