/*
Name: (NC) Schoolnet - Invalid LEA District Staff Roles
Description: This validation lists staff who have Schoolnet roles assigned at invalid "district" schools within an LEA. These rights should be assigned in the "Central Office" school. This will return no results in non-LEA PSUs.
...
WHERE
    ...
    AND (se.[schoolnetRole] IS NOT NULL OR se.[schoolnetAddRoles] IS NOT NULL)
    AND TRY_CAST(se.[SchoolNumber] as int) IS NOT NULL AND RIGHT(se.[SchoolNumber],3) = '000'
